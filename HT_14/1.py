    ...
    def add_bankomat_funds(self):
        ...
        try:
            ...
            if banknotes_num < 0:
                ...
                current_banknotes_total = cur.execute('SELECT banknote_total FROM banknotes WHERE banknote_value=?',
                                                      (banknotes_value,)).fetchone()
                if current_banknotes_total:
                    cur.execute('UPDATE banknotes SET banknote_total=? WHERE banknote_value=?',
                                (banknotes_num, banknotes_value))
                    con.commit()
                    con.close()
...
class UserOperations(object):
    ...
    def check_balance(self):
        ...
        con.close()
        return f'Текущий баланс пользователя {self.login} = {user_balance[0]}'
        ...
        try:
            ...
            if additional_funds < 0:
                ...
                cur.execute('UPDATE balance SET user_balance=? WHERE id=?',
                            (user_funds[0] + additional_funds, user_id[0]))
                con.commit()
                con.close()
                self.add_transaction('add', additional_funds)
    ...
    def drop_balance(self):
        ...
        try:
            ...
            self.add_transaction('drop', dropped_funds)
            result = ''
    ...
    def getcurrency_history(self, custom_date, currency):
        ...
        try:
            ...
            for i in self.daterange(date(yyyy, mm, dd)):
                time.sleep(1)
    ...
    def curr_converter(self,curr_from, curr_to, curr_value):
        try:
            ...
            if curr_from in curs_for_sale and curr_to in curs_for_sale:
                ...
                converted_value = [temp[i].get('saleRateNB') for i in range(len(temp)) if
                                   temp[i].get('currency') == curr_from]
...
class AuthCustom(object):
    ...
    def log_in(self):
        ...
        while True:
            ...
            access, incasator = self.login_pword_check(custom_login, custom_password)
            if access:
                self.start(custom_login, incasator)
                break
            else:
                try_counter -= 1
    ...
    def start(self, login, incasator=False):
        try:
            ...
            while True:
                if not incasator:
                    user_user = UserOperations(login)
                    ...
                    if user_operation in range(1, 8):
                        if user_operation == 1:
                            print(f'{"-" * 30}\n{user_user.check_balance()}')
                        elif user_operation == 2:
                            print(f'{"-" * 30}\n{user_user.add_balance()}')
                        elif user_operation == 3:
                            print(f'{"-" * 30}\n{user_user.drop_balance()}')
                        elif user_operation == 4:
                            print(f'{"-" * 30}\n{user_user.getcurrency_current()}')
                        elif user_operation == 5:
                            print('5 - История курса валют.')
                            currency = input('Введите дату (формат дд.мм.гггг): ')
                            custom_date = input(
                                'Введите валюту для просмотра (AZN, BYN, CAD, CHF, CNY, CZK, DKK, EUR, GBP, '
                                'HUF, ILS, JPY, KZT, MDL, NOK, PLN, RUB, SEK, SGD, '
                                'TMT, TRY, UAH, USD, UZS, GEL): ')
                            print(f'{"-" * 30}\n{user_user.getcurrency_history(currency, custom_date)}')
                            ...
                            print(f'{"-" * 30}\n{user_user.curr_converter(curr_from, curr_to, curr_value)}')
...
user_initiation = AuthCustom()

user_initiation.log_in()
